<template>
  <div class="p-6">
    <!-- Page Header -->
    <PageHeader title="Protection Approach"
      description="Define the steps and methodology for protecting the vulnerable." action-label="Add Approach Step"
      :action-icon="PlusIcon" @action="openCreateModal" />

    <!-- Stats -->
    <StatsGrid>
      <StatCard label="Total Steps" :value="stats.total" :icon="ShieldCheckIcon" color="orange" />
      <StatCard label="Active Steps" :value="stats.active" :icon="CheckCircleIcon" color="green" />
      <StatCard label="Hidden Steps" :value="stats.hidden" :icon="EyeSlashIcon" color="gray" />
    </StatsGrid>

    <!-- Loading State -->
    <LoadingState v-if="loading" message="Loading protection approaches..." />

    <!-- Empty State -->
    <EmptyState v-else-if="approaches.length === 0" title="No Protection Steps Defined"
      message="Outline the organization's protection strategy and systematic steps taken for interventions."
      action-label="Define First Step" :action-icon="PlusIcon" :icon="ShieldCheckIcon" @action="openCreateModal" />

    <!-- Approaches List -->
    <div v-else class="space-y-4">
      <div v-for="(approach, index) in approaches" :key="approach.id"
        ...
        <div
          class="w-16 h-16 flex-shrink-0 bg-orange-50 text-orange-600 rounded-2xl flex items-center justify-center font-bold text-2xl border border-orange-100 group-hover:bg-orange-600 group-hover:text-white transition-all">
          {{ index + 1 }}
        </div>

        <div class="flex-1 text-center md:text-left">
          <h3 class="text-xl font-bold text-gray-900 mb-1">{{ approach.title }}</h3>
          <p class="text-gray-600 line-clamp-2 md:line-clamp-1">{{ approach.description }}</p>
        </div>

        <div class="flex items-center gap-4">
          <span class="flex items-center gap-1.5 text-xs font-bold px-2 py-1 rounded-md"
            :class="approach.is_active ? 'bg-green-50 text-green-600' : 'bg-gray-100 text-gray-500'">
            <span class="w-1.5 h-1.5 rounded-full" :class="approach.is_active ? 'bg-green-500' : 'bg-gray-400'"></span>
            {{ approach.is_active ? 'Active' : 'Hidden' }}
          </span>
          <div class="flex gap-2">
            <button @click="editApproach(approach)"
              class="p-2 hover:bg-orange-50 rounded-lg text-gray-400 hover:text-orange-600 transition-colors">
              <PencilIcon class="h-5 w-5" />
            </button>
            <button @click="confirmDelete(approach)"
        ...
        <form @submit.prevent="handleSubmit" class="p-6 space-y-4">
          ...
          <div>
            ...
            <textarea v-model="form.description" required rows="4"
              class="w-full px-4 py-2 rounded-xl border border-gray-200 focus:ring-2 focus:ring-orange-600 outline-none transition-all"
              placeholder="Detailed description of what happens in this step..."></textarea>
          </div>
