<template>
  <div class="p-6">
    <!-- Page Header -->
    <PageHeader title="Organizational Team" description="Manage public profiles for the members of Sauti 116."
      action-label="Add Member" :action-icon="PlusIcon" @action="openCreateModal" />

    <!-- Stats -->
    <StatsGrid>
      <StatCard label="Total Members" :value="stats.total" :icon="UserGroupIcon" color="blue" />
      <StatCard label="Active Profiles" :value="stats.active" :icon="CheckCircleIcon" color="green" />
      <StatCard label="Hidden Profiles" :value="stats.hidden" :icon="EyeSlashIcon" color="gray" />
    </StatsGrid>

    <!-- Loading State -->
    <LoadingState v-if="loading" message="Loading team members..." />

    <!-- Empty State -->
    <EmptyState v-else-if="teamMembers.length === 0" title="Build Your Team"
      message="Add members of your organization to show the faces behind the Sauti 116 mission."
      action-label="Add First Member" :action-icon="PlusIcon" :icon="UserGroupIcon" @action="openCreateModal" />
      ...
      <div v-for="member in teamMembers" :key="member.id"
        ...
        <div class="p-5">
          ...
          <div class="mt-4 flex items-center justify-between">
            ...
            <span class="w-2 h-2 rounded-full" :class="member.is_active ? 'bg-green-500' : 'bg-gray-300'"
              :title="member.is_active ? 'Public' : 'Hidden'"></span>
          </div>
        </div>
      </div>
    </div>
