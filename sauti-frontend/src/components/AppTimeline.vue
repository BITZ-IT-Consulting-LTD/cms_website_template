<template>
  <div class="timeline-section relative">
    <div class="relative wrap overflow-hidden p-4 md:p-10 h-full">
      <!-- Vertical Line -->
      <div class="absolute border-opacity-30 border-sauti-blue h-full border-l-2 hidden md:block"
        style="left: 50%; transform: translateX(-50%);"></div>
      <div class="absolute border-opacity-30 border-sauti-blue h-full border-l-2 md:hidden" style="left: 2rem;">
      </div>
      ...
      <div v-for="(event, index) in sortedEvents" :key="event.id" class="mb-12 flex justify-between items-center w-full"
        ...
        <!-- Dot / Year Indicator -->
        <div
          class="z-20 flex items-center justify-center order-1 bg-sauti-white shadow-lg w-14 h-14 rounded-full border-4 border-sauti-blue transition-transform hover:scale-110 shrink-0">
          <span class="font-bold text-sm text-sauti-blue">{{ getYear(event) }}</span>
        </div>
