<template>
  <div class="chat-window" :class="{ 'chat-minimized': props.minimized }">
    ...
    <div ref="chatBodyEl" class="chat-body">
      ...
      <div v-if="currentQuestion && (currentQuestion.id === 'reporter_country' || currentQuestion.id === 'victim_country')" class="location-options">
      ...
      <div v-if="currentQuestion && currentQuestion.type === 'choice' && chatStage === 'standard-questions'" class="quick-actions">
        <button v-for="option in currentQuestion.options" :key="option" 
          @click.prevent="handleResponse(option)" class="action-button choice-button">
          {{ option }}
        </button>
      </div>

      <!-- Multiple Choice for Categories -->
      <div v-if="currentQuestion && currentQuestion.type === 'multiple_choice' && chatStage === 'standard-questions'" class="category-selection">
      ...
      <!-- File Upload Area -->
      <div v-if="currentQuestion && currentQuestion.type === 'file_upload' && chatStage === 'standard-questions'" class="file-upload-area">
    ...
    <div class="chat-footer">
      ...
      <!-- Standard Text Input -->
      <form v-if="showTextInput && currentQuestion && !['end', 'emergency_escalation', 'multiple_choice', 'file_upload'].includes(currentQuestion.type)" 
        @submit.prevent="handleSubmit">
        <div class="message-input-container">
          <textarea v-if="currentQuestion.type === 'textarea'" 
            v-model="userInput" 
            :placeholder="currentQuestion.placeholder || 'Share your story in detail...'"
          ...
          <input v-else
            ...
            :placeholder="currentQuestion.placeholder || getPlaceholderText(currentQuestion)"
            :disabled="isSubmitting" 
            class="message-input" />
...
.chat-window {
  ...
  width: 420px;
  height: 750px;
  background: rgba(255, 255, 255, 0.95); /* Transparent background */
...
.minimize-button, .maximize-button, .close-button, .back-button {
  background: none;
  border: none;
  color: #666;
  cursor: pointer;
...
.minimize-button:hover, .maximize-button:hover, .close-button:hover, .back-button:hover {
  opacity: 0.8;
}
...
@media screen and (min-width: 1200px) {
  .chat-window {
    width: 450px; /* Slightly bigger on large screens */
    height: 700px;
  }
}
...
@media screen and (min-width: 768px) and (max-width: 1199px) {
  .chat-window {
    width: 400px; /* Slightly smaller but still bigger than original */
    height: 700px;
    bottom: 15px;
    right: 15px;
    max-height: 80vh;
