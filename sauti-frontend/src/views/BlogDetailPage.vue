    ...
    <article v-else-if="post" class="bg-neutral-white">
      ...
      <div class="section-padding bg-neutral-white">
        <div class="container-custom max-w-4xl">
          ...
          <!-- Main Typography Content (collapsible for long articles) -->
          <div class="relative mb-24">
            <div class="prose-sauti max-w-none" :class="{ 'content-collapsed': isLongContent && !contentExpanded }"
              v-html="formattedContent"></div>
            <div v-if="isLongContent && !contentExpanded" class="content-fade"></div>
            <div v-if="isLongContent" class="flex justify-center mt-8">
              <button @click="contentExpanded = !contentExpanded"
                class="px-8 py-3 rounded-2xl bg-primary text-neutral-white font-bold text-sm hover:bg-secondary transition-all shadow-lg">
          ...
          <img v-if="post.secondary_image" :src="post.secondary_image" :alt="post.title"
            class="w-full rounded-[2rem] border-4 border-primary shadow-2xl mb-24" @error="setPlaceholder" />
            ...
            <div class="flex flex-col md:flex-row items-center justify-between gap-12">
              ...
              <div class="flex items-center gap-4">
                <button v-for="social in [
                  ...
                  <!-- Facebook -->
                  <svg v-if="social.icon === 'Facebook'" class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                  </svg>
                  <!-- Twitter / X -->
                  <svg v-else-if="social.icon === 'Twitter'" class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
...
<script setup>
  ...
  // Only offer the collapse/expand ("minimize / maximise") control for long
  // articles so short posts aren't awkwardly truncated.
  const isLongContent = computed(() => {
    const raw = String(post.value?.content || '')
    return raw.replace(/<[^>]+>/g, '').length > 1200
  })
...
<style scoped>
  ...
  :deep(.prose-sauti p) {
    @apply mb-10 text-justify;
  }

  /* Collapse/expand ("minimize / maximise") for long article bodies */
  .content-collapsed {
    max-height: 40rem;
    overflow: hidden;
  }
